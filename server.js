...
const db = mysql.createConnection({
  ...
  database: process.env.DB_NAME || "baron_local_db",
...
db.connect((err) => {
  if (err) {
    ...
    console.error("تأكد أن MySQL شغال وقاعدة baron_local_db موجودة.");
  } else {
    dbConnected = true;
    console.log("متصل بقاعدة البيانات: " + (process.env.DB_NAME || "baron_local_db"));
    ensureSettingsContactColumns();
  }
});

async function ensureSettingsContactColumns() {
  for (const column of ["instagram", "tiktok", "x"]) {
    try {
...
async function publishNotification({ type, title, message, related_id = null }) {
  ...
  try {
    ...
    const result = await q(
      "INSERT INTO notifications (id, type, title, content, is_seen, user_id) VALUES (?, ?, ?, ?, 0, ?)",
      [id, payload.type, payload.title, payload.message, related_id == null ? null : String(related_id)]
    );
    const rows = await q(
...
app.get("/api/health", (req, res) => {
  res.json({
    ...
    name: "الشام الذهبي",
    port: PORT,
    db: process.env.DB_NAME || "baron_local_db"
  });
});
...
app.post("/api/notifications/read-all", async (req, res) => {
  try {
    await q("UPDATE notifications SET is_seen = 1 WHERE is_seen = 0 OR is_seen IS NULL");
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/notifications", async (req, res) => {
  try {
    ...
    await q(
      "INSERT INTO notifications (id, type, title, content, is_seen, user_id) VALUES (?, ?, ?, ?, 0, ?)",
      [id, b.type || "system", b.title || "إشعار", b.message || "", b.related_id == null ? null : String(b.related_id)]
    );
    const rows = await q(
      "SELECT id, title, content AS message, type, is_seen AS is_read, created_at, user_id AS related_id FROM notifications WHERE id = ?",
      [id]
...
app.patch("/api/notifications/:id/read", async (req, res) => {
  try {
    await q("UPDATE notifications SET is_seen = 1 WHERE id = ?", [req.params.id]);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/settings", async (req, res) => {
  try {
    ...
    if (existing.length) {
      ...
      const result = await q(
        `INSERT INTO settings (site_name, logo, phone, whatsapp, telegram, facebook, instagram, tiktok, x, email, visits)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          b.site_name || "الشام الذهبي", b.logo || "", b.phone || "",
          b.whatsapp || "", b.telegram || "", b.facebook || "",
          b.instagram || "", b.tiktok || "", b.x || "",
          b.email || "", b.visits || 0
        ]
      );
      const rows = await q("SELECT * FROM settings WHERE id = ?", [result.insertId]);
      res.json(rows[0]);
    }
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.post("/api/bot-replies", async (req, res) => {
  try {
    ...
    const rows = await q("SELECT * FROM bot_replies WHERE id = ?", [result.insertId]);
    res.status(201).json(rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
